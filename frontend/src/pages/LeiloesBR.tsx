...
export default function LeiloesBR() {
  ...
  const { data: obras, isLoading, error } = useQuery({
    queryKey: ['obras', 'leiloes_br', page, categoria, artista],
    queryFn: () =>
      apiService.getObras({
        page,
        per_page: 50,
  ...
  const obrasFiltradas = useMemo(() => {
    ...
    return obras.filter((obra: any) => {
      ...
      // Filtro de leiloeiro
      if (leiloeiro && obra.leiloeiro) {
        if (!obra.leiloeiro.toLowerCase().includes(leiloeiro.toLowerCase())) return false
      }
      
      return true
    })
  }, [obras, valorMin, valorMax, leiloeiro])
  ...
  const temFiltrosAtivos = categoria || artista || valorMin || valorMax || leiloeiro
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      if (todasObrasData.length > 0) {
        const filename = `LeiloesBR_Obras_${new Date().toISOString().split('T')[0]}.xlsx`
          ...
          <div className="flex items-center gap-3">
            ...
            {temFiltrosAtivos && (
              <span className="bg-gradient-to-r from-[#c9a961] to-[#b89a4f] text-[#0f0f0f] text-xs px-3 py-1 rounded-full font-bold shadow-lg">
                {[categoria, artista, valorMin, valorMax, leiloeiro].filter(Boolean).length}
              </span>
            )}
          </div>
              ...
              <table className="w-full text-sm min-w-[800px]">
                <thead className="bg-gradient-to-r from-[#1a1a1a] to-[#151515] border-b border-[#2a2a2a]">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-[#c9a961] uppercase tracking-wider">Lote</th>
                    ...
                    <th className="px-6 py-4 text-left text-xs font-semibold text-[#c9a961] uppercase tracking-wider">Data Leilão</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-[#c9a961] uppercase tracking-wider">Leiloeiro</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-[#c9a961] uppercase tracking-wider">Local</th>
                    ...
                    <tr key={String(obra.id || obra.titulo || Math.random())} className="hover:bg-[#1a1a1a]/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-[#888] text-sm">
                          {obra.data_leilao && obra.data_leilao !== 'N/A' 
                            ? obra.data_leilao 
                      ...
                      <td className="px-6 py-4 max-w-[200px]">
                        <TruncatedText 
                          text={obra.leiloeiro && obra.leiloeiro !== 'N/A' ? obra.leiloeiro : 'N/A'} 
                          maxLength={25}
                      ...
                      <td className="px-6 py-4 max-w-[200px]">
                        <TruncatedText 
                          text={obra.local && obra.local !== 'N/A' ? obra.local : 'N/A'} 
                          maxLength={25}
                          className="text-[#888] text-sm"
                        />
                      </td>
        ...
        <div className="bg-gradient-to-br from-[#151515] to-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-16 text-center shadow-xl">
          ...
          <p className="text-sm text-[#888]">
            {temFiltrosAtivos
              ...
              : 'Execute um scraping do LeilõesBR para ver as obras aqui'}
          </p>
        </div>
      ) : null}
    </div>
